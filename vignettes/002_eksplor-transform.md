...
``` r
dim(iris) # mengetahui dimensi
```

    ## [1] 150   5

``` r
str(iris) # melihat struktur
```
...
``` r
summary(iris) # rangkuman statistik
```
...
``` r
length(levels(iris$Species)) # jumlah level kolom
```

    ## [1] 3

``` r
unique(iris$Species) # mengetahui jumlah uniq
```
...
``` r
cor(iris[, -5]) # ___>lainnya bisa dibaca dihelp google
```
...
``` r
library(visdat) #visualisasi
library(skimr) #summary visual
...
1.  memilih kolom
2.  antara tahun
3.  merubah bulan angka menjadi bulan nama
4.  grouping bulan itu
5.  menghitung rata2
